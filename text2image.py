...
def parse_args():
    ...
    # paths
    parser.add_argument("--config_path", type=str, default="configs/512_text.yaml", help="path to model config")
    parser.add_argument("--ckpt_path", type=str, default="pretrained/512_text.ckpt", help="path to model checkpoint")
    parser.add_argument("--save_folder", type=str, default="outputs/512_text2image", help="folder to save synthesis outputs")
...
def main():
    ...
    with torch.no_grad():
        ...
        with model.ema_scope("Plotting"):
            ...
            # encode condition
            condition = model.get_learned_conditioning(condition)  # [1, 77, 640]
            print(f'condition.shape={condition.shape}')  # [B, 77, 640]

            # DDIM sampling
            ddim_sampler = DDIMSampler(model)
            ddim_sampler = DDIMSampler(model); z_0_batch, intermediates = ddim_sampler.sample(S=args.ddim_steps, batch_size=args.batch_size, shape=(3, 64, 64), conditioning=condition, verbose=False, eta=1.0, log_every_t=1)

        # decode VAE latent z_0 to image x_0
        x_0_batch = model.decode_first_stage(z_0_batch)  # [B, 3, 256, 256]

        for idx in range(args.batch_size):
            writer.add_image(f"VAE的潜在空间代码z_0转换为图片x_0/{idx}", x_0_batch[idx])
    ...
    for idx in range(args.batch_size):
        ...
        writer.add_image(f'合成的图片x_0/{idx}', x_0_tensor, global_step=idx)
        ...
        if args.save_z:
            ...
            # ========== 保存 z_0 到 TensorBoard ==========
            z_0_tensor = torch.from_numpy(np.array(z_0).transpose(2, 0, 1))  # 转换为 [C, H, W] 的格式
            writer.add_image(f'潜在向量z_0/{idx}', z_0_tensor, global_step=idx)
